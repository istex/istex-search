...
describe("buildQueryFromIds", () => {
  ...
  it(`should identify any error but still replace '\n' by space and put each items in quotes`, () => {
    ...
    expect(Module.buildQueryFromIds("doi", ids)).toStrictEqual({
      errorLines: [],
      query:
        'doi.raw:("10.1136/bmj.2.2760.1392-a" "10.1136/jech.20.3.127" "10.1136/bmj.1.2091.212-a")',
    });
  });
});
...
describe("getIdsFromQuery", () => {
  it(`should replace ' OR ' by '\n' and remove quotes`, () => {
    const query = 'arkIstex:("id1" OR "id2" OR "id3")';
